---
summary: "All configuration options for ~/.joni/openclaw.json with examples"
read_when:
...
OpenClaw reads an optional **JSON5** config from `~/.joni/openclaw.json` (comments + trailing commas allowed).

If the file is missing, OpenClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.joni/workspace`). You usually only need a config to:
...
use `config.patch` or `openclaw config set`. Keep a backup of `~/.joni/openclaw.json`.
...
{
  agents: {
    defaults: { workspace: "~/.joni/workspace" },
    list: [
      {
        id: "main",
...
// ~/.joni/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.joni/workspace" }],
}
```
...
```json5
// ~/.joni/clients/mueller/agents.json5
[
...
```json5
// ~/.joni/clients/mueller/broadcast.json5
{
...
- `<agentDir>/auth-profiles.json` (default: `~/.joni/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.joni/credentials/oauth.json` (or `$JONI_STATE_DIR/credentials/oauth.json`)
...
- `~/.joni/agent/*` (migrated by `openclaw doctor` into `~/.joni/agents/<defaultAgentId>/agent/*`)
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.joni/credentials/whatsapp/biz
          // authDir: "~/.joni/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.joni/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.joni/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.joni/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.joni/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.joni/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.joni/workspace-home" },
      { id: "work", workspace: "~/.joni/workspace-work" },
    ],
  },
  bindings: [
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.joni/sandboxes`
...
`~/.joni/agents/<agentId>/agent/models.json` or by defining the same schema inside your
...
`~/.joni/agents/<agentId>/agent/` on startup:
...
- Override the agent config root with `OPENCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.joni/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.joni/agents/<agentId>/sessions/sessions.json
...
overrides. Applies to **bundled** skills and `~/.joni/skills` (workspace skills
...
from `~/.joni/extensions`, `<workspace>/.joni/extensions`, plus any
...
- OpenClaw.app watches `~/.joni/openclaw.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.joni/openclaw.json` (or `JONI_CONFIG_PATH`) and applies changes automatically.
...
- `~/.joni/openclaw.json` (or `JONI_CONFIG_PATH`)
...
- `JONI_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `openclaw --dev …` → uses `~/.joni-dev` + shifts ports from base `19001`
- `openclaw --profile <name> …` → uses `~/.joni-<name>` (port via config/env/flags)
...
```bash
JONI_CONFIG_PATH=~/.joni/a.json \
JONI_STATE_DIR=~/.joni-a \
...
Default root: `~/.joni/workspace/canvas`  
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.joni/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.joni/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.joni/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-gw._tcp` under `~/.joni/dns/` using the configured discovery domain (example: `openclaw.internal.`).
