...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.joni/dns/<domain>.db`

Validate from a tailnet‑connected machine:

```bash
dns-sd -B _openclaw-gw._tcp openclaw.internal.
dig @<TAILNET_IPV4> -p 53 _openclaw-gw._tcp.joni.internal PTR +short
```
...
- Set `gateway.bind: "tailnet"` in `~/.joni/openclaw.json`.
...
- `gateway.bind` in `~/.joni/openclaw.json` controls the Gateway bind mode.
