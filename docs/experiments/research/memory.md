...
read_when:
  - Designing workspace memory (~/.joni/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.joni/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.joni/workspace` as canonical human-readable memory.
...
```
~/.joni/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.joni/workspace`:
