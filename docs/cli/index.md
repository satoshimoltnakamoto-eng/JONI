...
- `--dev`: isolate state under `~/.joni-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.joni-<name>`.
...
- `--workspace <dir>`: agent workspace path (default `~/.joni/workspace`).
