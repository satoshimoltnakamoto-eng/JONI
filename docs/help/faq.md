...
- [Quick start and first-run setup]
  ...
  - [I can't access docs.joni.ai (SSL error). What now?](#i-cant-access-docsopenclawai-ssl-error-what-now)
...
2. Copy `$JONI_STATE_DIR` (default: `~/.joni`) from the old machine.
3. Copy your workspace (default: `~/.joni/workspace`).
...
under `~/.joni/` (for example `~/.joni/agents/<agentId>/sessions/`).
...
### I cant access docs.joni.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.joni.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.joni.ai`, then retry. More
...
(`~/.joni`) and workspace (`~/.joni/workspace`) stay untouched.
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.joni/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.joni/openclaw.json`). Precedence is `<workspace>/skills` > `~/.joni/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.joni/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.joni/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OpenClaw treats as `<workspace>/skills`.
...
Not directly. macOS skills are gated by `metadata.joni.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
...
3. Override the skill metadata (workspace or `~/.joni/skills`) to allow Linux:
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured OpenClaw workspace); OpenClaw treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.joni/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `JONI_HOME_VOLUME` so caches survive.
...
Everything lives under `$JONI_STATE_DIR` (default: `~/.joni`):

| Path                                                        | Purpose                                                      |
| ----------------------------------------------------------- | ------------------------------------------------------------ |
| `$JONI_STATE_DIR/openclaw.json`                             | Main config (JSON5)                                          |
| `$JONI_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$JONI_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$JONI_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$JONI_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$JONI_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
| `$JONI_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$JONI_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/.joni/agent/*` (migrated by `openclaw doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.joni/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.joni`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.joni`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.joni/skills`).

Default workspace is `~/.joni/workspace`, configurable via:
...
Do **not** commit anything under `~/.joni` (credentials, sessions, tokens).
...
OpenClaw reads an optional **JSON5** config from `$JONI_CONFIG_PATH` (default: `~/.joni/openclaw.json`):

```
$JONI_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.joni/workspace`).
...
- Daemons read env vars from `~/.joni/.env` (or the service environment).
...
- Restore from backup (git or a copied `~/.joni/openclaw.json`).
...
- a global fallback `.env` from `~/.joni/.env` (aka `$JONI_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.joni/.env` so they're picked up even when the service doesn't inherit your shell env.
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.joni-<profile>`).
...
- **Disk growth:** sessions + transcripts live under `~/.joni/agents/<agentId>/sessions/`.
...
- edit `agents.defaults.model` in `~/.joni/openclaw.json`
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.joni/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.joni/agent/*` (migrated by `openclaw doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.joni/.env` or enable `env.shellEnv`.
...
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.joni/.env` on the **gateway host**.
...
You're editing one config file while the service is running another (often a `--profile` / `JONI_STATE_DIR` mismatch).
...
- `JONI_CONFIG_PATH` (per-instance config)
- `JONI_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `openclaw --profile <name> …` per instance (auto-creates `~/.joni-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `openclaw --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.joni.*`, `openclaw-gateway-<profile>.service`, `OpenClaw Gateway (<profile>)`).
...
- macOS: `$JONI_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.joni/logs/...`; profiles use `~/.joni-<profile>/logs/...`)
