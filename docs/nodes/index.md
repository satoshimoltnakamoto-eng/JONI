...
- **Approvals**: enforced on the node host via `~/.joni/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.joni/openclaw.json` on the gateway host).
...
- `--display-name` on `openclaw node run` / `openclaw node install` (persists in `~/.joni/node.json` on the node).
...
Approvals live on the node host at `~/.joni/exec-approvals.json`.
...
- On headless node host, `system.run` is gated by exec approvals (`~/.joni/exec-approvals.json`).
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.joni/node.json`.
- Exec approvals are enforced locally via `~/.joni/exec-approvals.json`
