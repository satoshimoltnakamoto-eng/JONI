...
`~/.joni/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.joni/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.joni/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  ...
  - If you want to pre-download models manually (and warm the same index OpenClaw
    ...
    OpenClaw’s QMD state lives under your **state dir** (defaults to `~/.joni`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    OpenClaw uses:

    ```bash
    # Pick the same state dir OpenClaw uses
    STATE_DIR="${JONI_STATE_DIR:-$HOME/.joni}"
    if [ -d "$HOME/.moltbot" ] && [ ! -d "$HOME/.joni" ] \
      && [ -z "${JONI_STATE_DIR:-}" ]; then
...
- When `memory.qmd.sessions.enabled = true`, OpenClaw exports sanitized session
  ...
  `~/.joni/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.joni/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
...
- Session logs live on disk (`~/.joni/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
