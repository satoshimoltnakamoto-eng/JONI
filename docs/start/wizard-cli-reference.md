...
<Steps>
  <Step title="Existing config detection">
    - If `~/.joni/openclaw.json` exists, choose Keep, Modify, or Reset.
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.joni/.env` so launchd can read it.
...
- OAuth credentials: `~/.joni/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.joni/agents/<agentId>/agent/auth-profiles.json`
