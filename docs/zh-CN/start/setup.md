...
- **个性化设置存放在仓库之外：** `~/.joni/workspace`（工作区）+ `~/.joni/openclaw.json`（配置）。
...
- **配置：** `~/.joni/openclaw.json`（JSON/JSON5 格式）
- **工作区：** `~/.joni/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- **配对允许列表**：`~/.joni/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.joni/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.joni/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.joni/workspace` 和 `~/.joni/` 保持为"你的东西"；不要将个人提示/配置放入 `openclaw` 仓库。
