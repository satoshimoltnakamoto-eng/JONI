...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.joni/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.joni/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.joni`（或 `JONI_STATE_DIR`）
- 工作区：`~/.joni/workspace`（或 `~/.joni/workspace-<agentId>`）
- 智能体目录：`~/.joni/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.joni/agents/<agentId>/sessions`
...
- 工作区默认为 `~/.joni/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.joni/workspace-<profile>`）。
- 状态默认为 `~/.joni/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.joni/credentials/whatsapp/personal
