...
| `JONI_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
**Note:** With `JONI_STATE_DIR=/data`, the config path is `/data/openclaw.json`.
...
**Fix:** Ensure `JONI_STATE_DIR=/data` is set in `fly.toml` and redeploy.
