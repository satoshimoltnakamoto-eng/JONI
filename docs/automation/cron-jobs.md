...
- Jobs persist under `~/.joni/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.joni/cron/jobs.json` by default.
...
- Job store: `~/.joni/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.joni/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
