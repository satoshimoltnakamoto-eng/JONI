  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.joni/logs/openclaw.log" },
        hooks: {
          path: "~/.joni/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".joni", "logs", "openclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".joni", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".joni", "telegram.token"),
      );
