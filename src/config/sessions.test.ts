...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from JONI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { JONI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.JONI_STATE_DIR;
    process.env.JONI_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.JONI_STATE_DIR = prev;
      }
    }
  });
