...
describe("resolveEffectiveHomeDir", () => {
  it("prefers JONI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands JONI_HOME when set to ~", () => {
    const env = {
      JONI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for JONI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { JONI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when JONI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ JONI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { JONI_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
    });
