...
export function triggerOpenClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.JONI_SYSTEMD_UNIT, process.env.JONI_PROFILE);
  ...
  const label =
    process.env.JONI_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.JONI_PROFILE);
