...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.JONI_STATE_DIR;
  previousConfigPath = process.env.JONI_CONFIG_PATH;
  previousSkipBrowserControl = process.env.JONI_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.JONI_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.JONI_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.JONI_BUNDLED_PLUGINS_DIR;
...
function applyGatewaySkipEnv() {
  process.env.JONI_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.JONI_SKIP_GMAIL_WATCHER = "1";
  process.env.JONI_SKIP_CANVAS_HOST = "1";
  process.env.JONI_BUNDLED_PLUGINS_DIR = tempHome
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.JONI_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.JONI_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.JONI_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.JONI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.JONI_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.JONI_SKIP_CANVAS_HOST;
    } else {
      process.env.JONI_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.JONI_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.JONI_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.JONI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    delete process.env.JONI_GATEWAY_TOKEN;
  } else {
    process.env.JONI_GATEWAY_TOKEN = fallbackToken;
  }
