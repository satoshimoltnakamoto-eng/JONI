...
export function createGatewayReloadHandlers(params: {
  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.JONI_SKIP_GMAIL_WATCHER)) {
        try {
        ...
        params.logHooks.info("skipping gmail watcher restart (JONI_SKIP_GMAIL_WATCHER=1)");
      }
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.JONI_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.JONI_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (JONI_SKIP_CHANNELS=1 or JONI_SKIP_PROVIDERS=1)",
        );
      } else {
