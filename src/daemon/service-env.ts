...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.JONI_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.JONI_STATE_DIR;
  const configPath = env.JONI_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    JONI_PROFILE: profile,
    JONI_STATE_DIR: stateDir,
    JONI_CONFIG_PATH: configPath,
    JONI_GATEWAY_PORT: String(port),
    JONI_GATEWAY_TOKEN: token,
    JONI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    JONI_SYSTEMD_UNIT: systemdUnit,
    JONI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    JONI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    JONI_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    JONI_STATE_DIR: stateDir,
    JONI_CONFIG_PATH: configPath,
    JONI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    JONI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    JONI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    JONI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    JONI_LOG_PREFIX: "node",
    JONI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    JONI_SERVICE_KIND: NODE_SERVICE_KIND,
    JONI_SERVICE_VERSION: VERSION,
  };
}
