...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "JONI_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    ...
    process.env.JONI_STATE_DIR = path.join(home, ".joni");
    ...
    await fs.mkdir(path.join(home, ".joni"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.joni${path.sep}media`);
