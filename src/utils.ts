...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".joni");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.JONI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$JONI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via JONI_STATE_DIR.
