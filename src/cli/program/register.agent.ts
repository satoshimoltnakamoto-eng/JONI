...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.joni.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.joni.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openclaw agents set-identity --agent main --name "JONI" --emoji "🦞"', "Set name + emoji."],
  ...
  [
    "openclaw agents set-identity --identity-file ~/.joni/workspace/IDENTITY.md --agent main",
