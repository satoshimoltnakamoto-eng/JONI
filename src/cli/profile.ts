...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.joni${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.JONI_PROFILE = profile;

  const stateDir = env.JONI_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.JONI_STATE_DIR?.trim()) {
    env.JONI_STATE_DIR = stateDir;
  }
