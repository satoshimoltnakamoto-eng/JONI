...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_JONI_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_JONI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_JONI_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_JONI_BROWSER_ENABLED;
