...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep JONI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("joni security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation: "Prefer JONI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export async function collectPluginsCodeSafetyFindings(params: {
  ...
  const entries = await fs.readdir(extensionsDir, { withFileTypes: true }).catch((err) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and plugin layout, then rerun `joni security audit --deep`.",
    });
    return [];
  });
    ...
    for (const entry of entries) {
      ...
      const summary = await scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `joni security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
