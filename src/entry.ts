...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.JONI_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.JONI_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.JONI_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[joni] ${parsed.error}`);
    process.exit(2);
  }
